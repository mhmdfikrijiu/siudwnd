...
def download_one(slug: str, episode: int, work_dir: Path) -> Path:
    ...
    try:
        _download_episode_to_path(slug, episode, destination)
        return destination
    except Exception:
...
def download_full(
    ...
    try:
        ...
        def one(number: int) -> tuple[int, Path]:
            ...
            _download_episode_to_path(slug, number, path, lambda: report("retry", number))
            return number, path
        ...
        if not downloader.concat_mp4s([str(paths[number]) for number in numbers], str(output)):
...
def _download_episode_to_path(
    ...
    for attempt in range(3):
        try:
            ...
            if play_url and downloader.download_episode_hls(play_url, str(destination), referer):
